...
function Get-GitInfo {
    ...
    for ($d = $PWD.ProviderPath; $d; $d = [IO.Path]::GetDirectoryName($d)) {
        ...
        $candidateHead = [IO.Path]::Combine($g, "HEAD")
        if ([IO.Directory]::Exists($g) -and
            [IO.File]::Exists($candidateHead)) {
            $gitDir = $g
            break
        }
        if ([IO.File]::Exists($g)) {
...
function prompt {
    ...
    $parts = @()
    if ($sshInfo) { $parts += Format-Colored $sshInfo "91" }
    if ($gitInfo) { $parts += Format-Colored $gitInfo "94" }
    if ($kubeInfo) { $parts += Format-Colored $kubeInfo "96" }
