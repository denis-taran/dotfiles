...
sudo install -o root -g root -m 0755 "$SCRIPT_DIR/cloud-backup.sh" "$LIBEXEC_DIR/"
sudo install -o root -g root -m 0755 "$SCRIPT_DIR/cloud-cleanup.sh" "$LIBEXEC_DIR/"

sudo cp "$SCRIPT_DIR/cloud-backup@.service" "$UNIT_DIR/"
sudo cp "$SCRIPT_DIR/cloud-backup@.timer" "$UNIT_DIR/"

sudo systemctl daemon-reload
sudo systemctl enable --now "cloud-backup@$USER.timer"
