...
if $_is_root; then
    ...
    if [[ "$(id -u "$USERNAME")" == 0 ]]; then
        echo "Refusing to configure the root account. Run this script from your normal user with sudo." >&2
        exit 1
    fi
else
    USERNAME="${USER:-}"
...
declare -A links=(
    ...
    ["$SCRIPT_DIR/.npmrc"]="$HOMEDIR/.npmrc"
    ["$SCRIPT_DIR/.config/pip/pip.conf"]="$HOMEDIR/.config/pip/pip.conf"
